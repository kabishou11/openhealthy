...
function getOriginRecipeLink(sourcePath?: string): string {
  if (!sourcePath)
    return 'https://github.com/ryanuo/whatToEat'
  // Simple extraction of GitHub source path
  if (sourcePath.startsWith('http')) {
    return sourcePath
  }
  return `https://github.com/ryanuo/whatToEat/blob/master/app/dishes/${sourcePath}`
}
</script>
...
<template>
  ...
  <span v-else class="text-gray-900 font-bold">{{ nameText }}</span>
  <svg
    v-if="nameText !== '神马'"
    xmlns="http://www.w3.org/2000/svg"
    class="ml-1 align-base op-70 inline-block cursor-pointer"
    width="24"
    height="24"
    viewBox="0 0 24 24"
    @click="onClickVideo"
